		...
		<div id="main">
			<div class="section">
				...
				<?php SampleBody(
					...
					'<p>This function was created for my <a href="/portfolio/intuch">Intuch</a> project. The app allows users to look through their phone contacts, and choose which ones they want to receive reminder notifications for, if they haven\'t reached out in a while. For Intuch, I wanted to be able to send notificationswhen the app is closed, even though Android apps can normally only send notifications while they are running. I solved this issue by creating a custom Worker class, which can run in the background even if the app itself isn\'t.</p>
					<p>This particular Worker runs in the background once every day or so, looking through through the app\'s saved list of contacts, checking the last time the user texted them, and checking if it they want a reminder to reach out. If so, it creates a reminder notification for that contact and pushes it to the device. If several contacts require a notification, they are grouped together nicely under a summary notification.</p> 
